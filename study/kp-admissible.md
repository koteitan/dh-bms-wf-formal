...
落とした理由は 1 つである。$`\Delta_0`$ に収まらないものを外してある。
...
無限だけが落ちる。帰納的集合 $`x`$ は $`\emptyset, \{\emptyset\}, \{\emptyset,\{\emptyset\}\}, \dots`$ を
...
$`v_0`$ に $`x`$ を入れて $`a`$ と繋いだ完全な割り当てを $`a_x`$ と書く。
$`T`$ に入るのは $`a_x`$ のほうである。
...
原文 §10 は、KP から必要な道具を順に組み立てる。連鎖は次のとおりである。
