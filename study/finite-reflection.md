...
**$`\omega_1^V`$.** 外部宇宙 $`V`$ での最小の非可算順序数。

**正則性.** $`\omega_1`$ は正則である。すなわち、$`\omega_1`$ 未満の順序数からなる可算個の族の
上限は $`\omega_1`$ 未満である。

**$`\lt_L`$.** $`L`$ には定義可能な整列順序 $`\lt_L`$ が入る。各元を「どの段で、どの論理式コードと
どのパラメータから作られたか」で並べればよい。これにより「条件を満たす最小の元」を
選択公理なしに指定できる。

**Skolem 関数.** 構造 $`N`$ と論理式 $`\exists x\, \varphi(x, \bar y)`$ に対し、$`\bar y`$ を受け取って
$`\varphi`$ の証人 $`x`$ を 1 つ返す関数。$`\lt_L`$-最小の証人を返すことにすれば一意に決まる。

**Tarski–Vaught test.** 推移的な $`M \subseteq N`$ について、$`N`$ で真な
$`\exists x\, \varphi(x, \bar a)`$（$`\bar a \in M`$）に必ず $`M`$ の中の証人があるなら
