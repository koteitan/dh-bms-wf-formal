...
**定義（$`\lt_L`$）.** $`L`$ の元 $`x`$ に対し、$`x`$ が初めて現れる段を $`\mathrm{st}(x)`$ と書く。
$`x \lt_L y`$ を次で定める。

- $`\mathrm{st}(x) \lt \mathrm{st}(y)`$ のとき
- $`\mathrm{st}(x) = \mathrm{st}(y) = \xi + 1`$ で、$`x`$ を $`\mathrm{Def}(L_\xi)`$ から
  切り出す対 $`\langle e, a \rangle`$ が、$`y`$ を切り出す対より小さいとき

第 2 の場合の $`\langle e, a \rangle`$ どうしの比較は、$`e`$ を自然数として、$`a`$ の各成分を
$`L_\xi`$ 上の $`\lt_L`$ で比べる。全体は $`\xi`$ に沿った超限再帰である。
