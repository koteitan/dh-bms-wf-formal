...
集合 $`X`$ を一つ取る。**構造** $`(X, \in)`$ とは、

- 変数の動く範囲を $`X`$ とし、
- 記号 $`\in`$ を本物の帰属関係を $`X`$ に制限したもの

と読む約束のことである。

論理式 $`\varphi`$ の自由変数へ $`X`$ の元を割り当てたものを $`\bar a`$ と書き、

```math
(X, \in) \models \varphi[\bar a]
```

を「割り当て $`\bar a`$ のもとで $`\varphi`$ は構造 $`(X,\in)`$ で真である」と読む。
$`\models`$ は「充足する」と読む記号である。

意味は論理式の形に沿って次で定まる。$`\bar a(z \mapsto c)`$ は「$`z`$ の値だけ $`c`$ に
差し替えた割り当て」を表す。
